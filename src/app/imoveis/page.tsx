...
export default async function PaginaDetalheImovel({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 2. Proteção contra IDs inválidos (UUID)
  ...
  // Se der erro no banco ou não achar o imóvel
  if (error || !imovel) {
    return notFound();
  }
  ...
  const linkWhatsApp = `https://wa.me/5511999999999?text=${textoWhatsApp}`; // Troque pelo seu número
      ...
      <div className="h-[50vh] md:h-[65vh] bg-terras-marrom/5 relative">
         {imovel.imagens && imovel.imagens.length > 0 ? (
           <div className="grid grid-cols-1 md:grid-cols-4 grid-rows-2 h-full gap-1 p-1">
             ...
             {/* Outras Imagens (Menores) - AQUI ESTAVA O ERRO DO 'ANY', AGORA CORRIGIDO */}
